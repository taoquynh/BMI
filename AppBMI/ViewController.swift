...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let gesture1 = UITapGestureRecognizer(target: self, action: #selector(decreaseWeight(_:)))
        weightMinusIcon.addGestureRecognizer(gesture1)
        
        let gesture2 = UITapGestureRecognizer(target: self, action: #selector(increaseWeight(_:)))
        weightPlusIcon.addGestureRecognizer(gesture2)
        
        let gesture3 = UITapGestureRecognizer(target: self, action: #selector(decreaseAge(_:)))
        ageMinusIcon.addGestureRecognizer(gesture3)
        
        let gesture4 = UITapGestureRecognizer(target: self, action: #selector(increaseAge(_:)))
        agePlusIcon.addGestureRecognizer(gesture4)
        
        let longGesture1 = UILongPressGestureRecognizer(target: self, action: #selector(onLongPressIncreaseWeight(_:)))
        weightPlusIcon.addGestureRecognizer(longGesture1)
        
        let longGesture2 = UILongPressGestureRecognizer(target: self, action: #selector(onLongPressDecreaseWeight(_:)))
        weightMinusIcon.addGestureRecognizer(longGesture2)
        
        let longGesture3 = UILongPressGestureRecognizer(target: self, action: #selector(onLongPressIncreaseAge(_:)))
        agePlusIcon.addGestureRecognizer(longGesture3)
        
        let longGesture4 = UILongPressGestureRecognizer(target: self, action: #selector(onLongPressDecreaseAge(_:)))
        ageMinusIcon.addGestureRecognizer(longGesture4)
    ...
    func setupLayout(){
        ...
        titleView.heightAnchor.constraint(equalToConstant: topPadding + 40).isActive = true
        ...
        calculateButton.bottomAnchor.constraint(equalTo: contentView.bottomAnchor, constant: -(bottomPadding + margin)).isActive = true
    ...
    // tăng cân
    @objc func increaseWeight(_ sender: UITapGestureRecognizer){
        if weight >= maxWeight {
            weight = maxWeight
        } else {
            weight = weight + 1
    ...
    // giảm cân
    @objc func decreaseWeight(_ sender: UITapGestureRecognizer){
        if weight <= minWeight {
            weight = minWeight
        } else {
            weight = weight - 1
    ...
    // tăng tuổi
    @objc func increaseAge(_ sender: UITapGestureRecognizer){
        if age >= maxAge {
            age = maxAge
        } else {
            age = age + 1
    ...
    // giảm tuổi
    @objc func decreaseAge(_ sender: UITapGestureRecognizer){
        if age <= minAge {
            age = minAge
        } else {
            age = age - 1
    ...
    @objc func onLongPressIncreaseWeight(_ gesture: UILongPressGestureRecognizer) {
        ...
        case .began:
            timer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(increaseWeight(_:)), userInfo: nil, repeats: true)
        case .ended:
            timer.invalidate()
        default:
            break
    ...
    @objc func onLongPressDecreaseWeight(_ gesture: UILongPressGestureRecognizer) {
        ...
        switch gesture.state {
        case .began:
            timer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(decreaseWeight(_:)), userInfo: nil, repeats: true)
        case .ended:
            timer.invalidate()
        default:
            break
    ...
    @objc func onLongPressIncreaseAge(_ gesture: UILongPressGestureRecognizer) {
        ...
        switch gesture.state {
        case .began:
            timer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(increaseAge(_:)), userInfo: nil, repeats: true)
        case .ended:
            timer.invalidate()
        default:
            break
    ...
    @objc func onLongPressDecreaseAge(_ gesture: UILongPressGestureRecognizer) {
        ...
        switch gesture.state {
        case .began:
            timer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(decreaseAge(_:)), userInfo: nil, repeats: true)
        case .ended:
            timer.invalidate()
        default:
            break
